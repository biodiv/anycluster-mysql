...
class MapClusterer():
    ...
    def convertCellsToGEOS(self,cells):
        ...
        for counter, cell in enumerate(cells):
            ...
            geospoly = self.clusterCellToBounds(cell)
                
            if counter > 0:
                query_collection += ","

            # ST_GeomFromText('POLYGON((0 0, 10000 0, 10000 10000, 0 10000, 0 0))',3857)
            query_collection += "ST_GeomFromText('%s')" % (geospoly.ewkt)
    ...
    def getClusterGeometries(self, request, params, clustertype):
        ...
        if geometry_type == "viewport":
            ...
            for cell in clusterGeometries_pre:
                cell_geos = self.clusterCellToBounds(cell)
                clusterGeometries.append({"geos": cell_geos, "k": BASE_K, "cellid": json.dumps(cell)})
    ...
    def distanceCluster(self, clusters, c_distance=30):
        ...
        for cluster in clusters:
            clustercoords = cluster["geojson"]

            added = False

            for processed_cluster in clusters_processed:
                processed_coords = processed_cluster["geojson"]
                ...
                if pixel_distance <= c_distance:
                    new_x = clustercoords.x + ( ( processed_coords.x - clustercoords.x) / 2 )
                    new_y = clustercoords.y + ( ( processed_coords.y - clustercoords.y) / 2 )
                    new_coords = Point(new_x, new_y, srid= clustercoords.srid)

                    processed_cluster["geojson"] = new_coords
                    
                    processed_cluster["ids"].append(cluster["ids"])
    ...
    def centroidCluster(self, request):
        ...
        if clusterGeometries:
            ...
            for geometry_dic in clusterGeometries:
                ...
                #cursor.execute('''SELECT count(*), AsText(mysql.ST_ConvexHull(%s)) FROM %s
                #                    WHERE coordinates IS NOT NULL AND Within( %s, GeomFromText( '%s' ) ) 
                #                ''' % ( geo_column_str, geo_table, geo_column_str, geos_geometry.wkt) )

                cursor.execute('''SELECT count(*), AsText(Envelope(%s)) FROM %s
                                    WHERE coordinates IS NOT NULL AND Within( %s, GeomFromText( '%s' ) ) 
                                ''' % ( geo_column_str, geo_table, geo_column_str, geos_geometry.wkt) )
                

                #cursor.execute('''SELECT count(*), AsText( mysql.ST_Centroid(%s)) FROM %s
                #                    WHERE coordinates IS NOT NULL AND Within( %s, GeomFromText( '%s' ) ) 
                #                ''' % ( geo_column_str, geo_table, geo_column_str, geos_geometry.wkt) )
                ...
                if poly:
                        ...
                        if poly[0] and poly[1]:
                            ...
                            hull = GEOSGeometry(poly[1], srid = self.srid_db)

                            centroid = hull.centroid

                            if centroid.srid != self.input_srid:
                                    centroid.transform(self.input_srid)

                            markers.append({'ids': [geometry_dic["cellid"]], 'count': count, "geojson": centroid, 'center': {
                                            'x': centroid.x, 'y': centroid.y}, 'pinimg': None})


        markers = self.distanceCluster(markers)

        for m in markers:
            m["geojson"] = m["geojson"].wkt
    ...
    def clusterCellToBounds(self, cell):
        ...
        geospoly = GEOSGeometry(poly, srid=self.srid_db)
    ...
    def getKmeansClusterContent(self, request, custom_filterstring = ""):
        ...
        entries_queryset = Gis.objects.raw('''
                    ...
                    ''' %(geo_column_str, geo_column_str, BASE_K, geo_table,
                          geo_table, geo_column_str, geo_column_str, geospoly.ewkt, filterstring,
                          kmeans_string ))

        return entries_queryset


    def getCentroidContent(self, request):
        ...
        for cell in celllist:
            geospoly = self.clusterCellToBounds(json.loads(cell))
            cells.append(geospoly)

        entries = Gis.objects.filter(coordinates__intersects=cells)
